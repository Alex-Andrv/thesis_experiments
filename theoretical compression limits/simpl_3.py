...
for k in tqdm(range(0, cnt_row + 1)):
    ...
    for i, zero in enumerate(combinations(list(range(1 << len(vars))), k)):
        ...
        variables_1.append(model.addVar(vtype="I", name=f"b1", lb=lb, ub=ub))

        variables_2 = []
        for var in vars:
            variables_2.append(model.addVar(vtype="I", name=f"{var}_2", lb=lb, ub=ub))

        variables_2.append(model.addVar(vtype="I", name=f"b2", lb=lb, ub=ub))

        variables_3 = []
        for var in vars:
            variables_3.append(model.addVar(vtype="I", name=f"{var}_3", lb=lb, ub=ub))

        variables_3.append(model.addVar(vtype="I", name=f"b3", lb=lb, ub=ub))
